...
namespace video
{
	...
	public:
		...
		//! Sets the MinFilter, MagFilter and AnisotropicFilter fields according
		//! to boolean values.
		/** The value of `trilinear` takes precedence over the value of `bilinear`.
		This function exists because ...
		A. a lot of Minetest code used to set the BilinearFilter and TrilinearFilter
		   boolean fields (which no longer exist),
		B. a lot of Minetest code used to set the AnisotropicFilter u8 field as
		   a boolean via SMaterial::setFlag (which also no longer exists),
		and C. I didn't want to rethink all that code. */
		void setFiltersLegacy(bool bilinear, bool trilinear, bool anisotropic) {
