	...
	void COpenGL3DriverBase::setTextureRenderStates(const SMaterial& material, bool resetAllRenderstates)
	{
		...
		for (s32 i = Feature.MaxTextureUnits - 1; i >= 0; --i)
		{
			...
			if (!tmpTexture->getStatesCache().IsCached || material.TextureLayer[i].MinFilter != tmpTexture->getStatesCache().MinFilter ||
				material.TextureLayer[i].MagFilter != tmpTexture->getStatesCache().MagFilter)
			{
				...
				tmpTexture->getStatesCache().MinFilter = material.TextureLayer[i].MinFilter;
				tmpTexture->getStatesCache().MagFilter = material.TextureLayer[i].MagFilter;
			}

			if (material.UseMipMaps && tmpTexture->hasMipMaps())
			{
				if (!tmpTexture->getStatesCache().IsCached || material.TextureLayer[i].MinFilter != tmpTexture->getStatesCache().MinFilter ||
					material.TextureLayer[i].MagFilter != tmpTexture->getStatesCache().MagFilter || !tmpTexture->getStatesCache().MipMapStatus)
				{
